...
#task role for ecs task, which will be used to access dynamodb
resource "aws_iam_role" "ecs_task_role" {
  name = "c22-dashboard-divas-ecs-task-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
...
#cloudwatch log group for dashboard service
resource "aws_cloudwatch_log_group" "dashboard_log_group" {
    name = "/ecs/c22-dashboard-divas-dashboard-service"
    retention_in_days = 7
}
